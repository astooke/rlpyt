...
from rlpyt.ul.pretrain_algos.data_augs import quick_pad_random_crop, intensity
...
ValInfo = namedtuple("ValInfo", ["cpcLoss", "accuracy", "fc1Activation", "zActivation"])
...
class CPC(UlAlgorithm):
    ...
    def initialize(self, n_updates, cuda_idx=None):
        ...
        if self.positives == "TB_1_4":
            self.transform_2 = copy.deepcopy(self.transform)  # start with same weights?
            self.transform_2.to(self.device)
    ...
    def optimize(self, itr):
        ...
        self.optimizer.zero_grad()
        cpc_loss, accuracy, z_anchor, fc1_anchor = self.cpc_loss(samples)
    ...
    def cpc_loss(self, samples):
        if self.replay_sequence_length == 0:
            ...
            if self.positives == "anchor":
                ...
                if self.positives == "TB_1_4":
                    ...
                    anchor = samples.observation[:-4]  # will down-select after random crop
                    anchor = anchor.reshape((t - 4) * b, c, h, w)  # to random crop different by time step
                    positive = samples.observation[1:]
                    positive = positive.reshape((t - 1) * b, c, h, w)
                else:
        ...
        if self.intensity_sigma > 0 and self.intensity_prob > 0:
            anchor = intensity(anchor, sigma=self.intensity_sigma, prob=self.intensity_prob)
            positive = intensity(positive, sigma=self.intensity_sigma, prob=self.intensity_prob)
        ...
        if self.positives == "TB_1_4":
            z_positive_TB = z_positive.reshape((t - 1), b, -1)
            z_positive_1 = z_positive_TB[:-3].reshape((t - 4) * b, -1)
            z_positive_2 = z_positive_TB[3:].reshape((t - 4) * b, -1)
            if action is not None:
                raise NotImplementedError
            logits_1 = self.transform(z_anchor, z_positive_1, action, reward)
            logits_2 = self.transform_2(z_anchor, z_positive_2, action, reward)
            labels_1 = torch.arange(z_anchor.shape[0],
                dtype=torch.long, device=self.device)
            labels_2 = torch.arange(z_anchor.shape[0],
                dtype=torch.long, device=self.device)
            invalid = ~valid_from_done(samples.done).type(torch.bool)
            nvld_1 = invalid[1:-3].reshape(-1)  # from where the positives are
            nvld_2 = invalid[4:].reshape(-1)
            labels_1[nvld_1] = IGNORE_INDEX
            labels_2[nvld_2] = IGNORE_INDEX
            loss_1 = self.c_e_loss(logits_1, labels_1)
            loss_2 = self.c_e_loss(logits_2, labels_2)
            loss = loss_1 + loss_2  # ooooo divide by 2?
            correct = torch.argmax(logits_2.detach(), dim=1) == labels_2
            accuracy = torch.mean(correct[~nvld_2].float())
    ...
    def state_dict(self):
        ...
        if self.positives == "TB_1_4":
            state_dict["transform_2"] = self.transform_2.state_dict()
        return state_dict
    ...
    def load_state_dict(self, state_dict):
        ...
        if self.positives == "TB_1_4":
            self.transform_2.load_state_dict(state_dict["transform_2"])

    def parameters(self):
        yield from self.encoder.parameters()
        yield from self.transform.parameters()
        if self.positives == "TB_1_4":
            yield from self.transform_2.parameters()
    ...
    def named_parameters(self):
        ...
        if self.positives == "TB_1_4":
            yield from self.transform_2.named_parameters()

    def eval(self):
        self.encoder.eval()  # in case of batch norm
        self.transform.eval()
        if self.positives == "TB_1_4":
            self.transform_2.eval()

    def train(self):
        self.encoder.train()
        self.transform.train()
        if self.positives == "TB_1_4":
            self.transform_2.train()
